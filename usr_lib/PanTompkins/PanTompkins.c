...
int16_t PT_StateMachine(int16_t datum)
{
	...
	if (!PEAKI && BlankTimeCnt)								// No beat, decrement BlankTime，即检测不到心�?
	{
		if (--BlankTimeCnt == 0)							// If blanktime over place the oldest peak
			PEAKI = PEAKI_temp;
	}
	else if (PEAKI && !BlankTimeCnt)						// If no peak for peak for last 200msec, save the current peak，过去的200ms内没有�?�测到峰，则�?�为这个是R
	{
		BlankTimeCnt = PT200MS;
		PEAKI_temp   = PEAKI;
		PEAKI = 0;
	}
	else if(PEAKI)											// If a bigger peak comes along, store it，遇到一�?更高的峰值，则保存�?�峰�?
	{
		if (PEAKI > PEAKI_temp)
		{
			BlankTimeCnt = PT200MS;
...
uint16_t PT_get_SPKI_output(void) {
	return (PT_dptr->SPKI);
}
